...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(
        ...
        # Residual Connection. #
        dec_res_output = tf.add(
            x_embed, attn_self_output + ffwd_self_output)
        
        # Encoder-Decoder Attention. #
        # Pre Layer Normalization. #
        ffwd_self_norm = self.lnorm_3(dec_res_output)
        ...
        # Residual Connection. #
        enc_dec_res_output = tf.add(
            dec_res_output, attn_enc_dec_output + ffwd_enc_dec_output)
        return enc_dec_res_output
...
class Transformer(tf.keras.Model):
    ...
    def call(self, x_encode, x_decode, training=True):
        ...
        w_embed = self.decoder_model.dec_embed(
    ...
    def compute_xent_loss(self, x_encode, x_decode, x_label):
        seq_length  = x_decode.shape[1]
        enc_outputs = self.encoder_model(
            x_encode, training=True)
        dec_outputs = self.decoder_model(
            x_decode, enc_outputs, training=True)
        ...
        for t_step in range(seq_length):
            dec_logit = tf.matmul(
                dec_outputs[:, t_step, :], w_embed, transpose_b=True)
        ...
        # Concatenate the losses across the sequence. #
