...
def train_step(
    ...
    for n_sub in range(n_sub_batch):
        ...
        with tf.GradientTape() as grad_tape:
            raw_losses = model.compute_xent_loss(
                tmp_encode, tmp_decode, tmp_output)
    ...
    optimizer.apply_gradients(zip(clip_tuple[0], model_params))
    return avg_losses
...
model_ckpt_dir  = "../TF_Models/dialogue_sw_transformer"
train_loss_file = "train_loss_dialogue_sw_transformer.csv"
...
while n_iter < maximum_iter:
    ...
    tmp_loss = train_step(
        seq2seq_model, seq2seq_optim, sub_batch, 
        tmp_input, tmp_decode, tmp_output, 
        learning_rate=learn_rate_val, grad_clip=gradient_clip)
